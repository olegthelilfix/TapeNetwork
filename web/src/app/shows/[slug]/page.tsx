...
import type { ShowDetail } from "@/lib/api/types";
import { mediaUrl, formatDate } from "@/lib/format";
import styles from "./show.module.css";

export const dynamic = "force-dynamic";

async function load(slug: string): Promise<ShowDetail> {
  try {
...
export default async function ShowPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const stats = [
    { v: s.episodesCount ? s.episodesCount.toLocaleString("en-US") : "—", k: "Episodes" },
      ...
      <div className={styles.layout}>
        <section className={styles.episodes}>
          <div className={styles.tabs}>
            ...
            <span className={styles.count}>{s.episodesCount.toLocaleString("en-US")} episodes</span>
          </div>
              ...
              <li key={e.slug}>
                <Link href={`/watch/${e.slug}`} className={styles.epRow}>
                  <div className={styles.epMedia}>
                    {mediaUrl(e.imageUrl) && (
                      <Image src={mediaUrl(e.imageUrl)!} alt="" fill sizes="200px" className={styles.epImg} />
                    )}
                    {e.live && <span className={styles.epLive}>LIVE</span>}
                  </div>
                  <div className={styles.epBody}>
                    <div className={styles.epMeta}>
                      {[e.epNo, formatDate(e.publishedAt), e.views].filter(Boolean).join(" · ")}
                    </div>
