...
import type { HomeResponse, ShowSummary } from "@/lib/api/types";
...
function kickerFor(refType: string, live: boolean): string | undefined {
  if (live) return undefined; // the LIVE badge is shown instead
  return refType === "video" ? "CLIP" : "REPLAY";
}
...
export default async function HomePage() {
  ...
  try {
    [data, shows] = await Promise.all([api.home(), api.shows()]);
  } catch {
    return (
  ...
  const heroSrc = mediaUrl(hero?.imageUrl);
            ...
            <Link href={`/watch/${hero.slug}`} className={styles.hero}>
              <div className={styles.heroMedia}>
                ...
                <div className={styles.heroBadges}>
                  {hero.live && <span className={styles.onAir}>● ON AIR</span>}
                  <span className={styles.featuredTag}>FEATURED</span>
                </div>
                {hero.live && hero.views && <span className={styles.watching}>{hero.views}</span>}
                ...
                <div className={styles.heroText}>
                  ...
                  <div className={styles.heroByline}>
                    {hero.live ? "Live from the floor" : hero.durationLabel}
                  </div>
                </div>
              </div>
            </Link>
            ...
            <div className={styles.featuredGrid}>
              {data.featured.map((c) => (
                <Card
                  ...
                  title={c.title}
                  subtitle={c.subtitle}
                  kicker={kickerFor(c.refType, c.live)}
                ...
                <li key={i} className={styles.slot}>
                  ...
                  <span className={styles.slotBody}>
                    <span className={styles.slotName}>
                      ...
                      {s.live && <span className={styles.slotLive}>LIVE</span>}
                    </span>
          ...
          {leadArticle && (
            <Link href={`/articles/${leadArticle.slug}`} className={styles.lead}>
              <div className={styles.leadMedia}>
                {mediaUrl(leadArticle.imageUrl) && (
                  <Image src={mediaUrl(leadArticle.imageUrl)!} alt="" fill sizes="(max-width: 900px) 100vw, 560px" className={styles.leadImg} />
                )}
