...
- **Flyway owns the schema.** `spring.jpa.hibernate.ddl-auto=none`. Migrations are **forward-only** — never edit an applied `V*.sql`; add a new `V<n>__*.sql`. Beware: `${...}` is a Flyway **placeholder** even inside SQL comments — don't write literal `${FOO}` in a migration or it fails to parse.
  - **Dev DB self-heal.** Local Postgres volumes outlive branch switches, so a changed checksum or a rolled-back migration would abort boot. The compose dev stack sets `FLYWAY_REPAIR_ON_MIGRATE=true` (runs `repair()` before `migrate()`, via `FlywayConfig`) and `FLYWAY_IGNORE_MIGRATION_PATTERNS=*:missing,*:future`. Both default **off** in `application.yml` — production keeps strict validate-then-migrate. To fix a stuck DB by hand: `podman compose down -v` (dev), or `delete from flyway_schema_history where version='<n>'` then restart.
