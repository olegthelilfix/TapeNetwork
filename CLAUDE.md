...
- **Flyway owns the schema.** `spring.jpa.hibernate.ddl-auto=none`. Migrations are **forward-only** — never edit an applied `V*.sql`; add a new `V<n>__*.sql`. Beware: `${...}` is a Flyway **placeholder** even inside SQL comments — don't write literal `${FOO}` in a migration or it fails to parse.
  - **Dev DB self-heal (`FlywayConfig`).** Persistent DBs outlive branch switches, so a changed checksum or a rolled-back migration aborts boot. `FLYWAY_STRATEGY` (default `none` = strict, production) picks a non-prod startup mode: `repair` runs `repair()` before `migrate()` (heals checksum drift, keeps data — the compose dev default); `clean` runs `clean()` before `migrate()` to rebuild the schema from `V1..V<n>` every boot (ephemeral test stacks — **drops all data**, also needs `FLYWAY_CLEAN_DISABLED=false`). `FLYWAY_IGNORE_MIGRATION_PATTERNS` (e.g. `*:missing,*:future`) tolerates rollbacks. To fix a stuck DB by hand: `podman compose down -v` (dev), or `delete from flyway_schema_history where version='<n>'` then restart.
